  ...
  return (
    <div className="min-h-screen bg-[#05070A] font-['Plus_Jakarta_Sans',sans-serif] text-white p-3 md:p-4 relative overflow-hidden">
      ...
      <div className="relative max-w-[1500px] mx-auto flex flex-col gap-3 animate-in fade-in duration-500">
        {/* Tabs + Period (compact, combined header) */}
        <div className="flex items-center gap-2 border-b border-white/[0.06] pb-1 overflow-x-auto">
          <div className="flex items-center gap-2 pr-3 mr-1 border-r border-white/[0.06]">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#3B82F6] to-[#1E3A8A] flex items-center justify-center ring-1 ring-white/10">
              <Trophy className="w-3.5 h-3.5 text-white" strokeWidth={2} />
            </div>
            <h1 className="text-sm font-bold tracking-tight whitespace-nowrap hidden sm:block">
              Dashboard <span className="text-white/50 font-medium">· {activeMonth === "all" ? "Todos" : activeMonth}</span>
            </h1>
          </div>
          {([
            ["geral", "Visão Geral"],
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Select value={activeMonth} onValueChange={setMonth}>
              <SelectTrigger className="w-[150px] bg-white/[0.03] border border-white/10 text-white/80 rounded-lg h-7 text-xs backdrop-blur-md">
        ...
        <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
          {kpis.map((k, i) => (
            <motion.div
              ...
              className={cn(
                "group relative bg-white/[0.03] border px-2.5 py-2 rounded-lg backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5",
                k.highlight
                  ? "border-[#F97316]/50 shadow-[0_0_20px_-8px_rgba(249,115,22,0.5)]"
                  : "border-white/10 hover:border-white/20",
              )}
            >
              <div className="flex justify-between items-center gap-1 mb-1">
                <p className="text-white/50 text-[8px] font-bold uppercase tracking-[0.12em] truncate">{k.l}</p>
                <k.Icon className="w-3 h-3 shrink-0" strokeWidth={2.4} style={{ color: k.color }} />
              </div>
              <p className={cn("font-bold text-white tabular-nums tracking-tight leading-none", k.small ? "text-xs" : "text-lg")}>{k.v}</p>
              {k.bar !== undefined && (
                <div className="w-full bg-white/[0.06] h-0.5 rounded-full mt-1.5 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
              ...
              {k.highlight && (
                <span className="absolute -top-1.5 right-1 bg-gradient-to-r from-[#F97316] to-[#000] text-white text-[7px] px-1.5 py-[1px] font-black rounded-full uppercase tracking-widest shadow-lg shadow-orange-500/50 border border-orange-400/40">
                  Poss. Venda
                </span>
              )}
            </motion.div>
