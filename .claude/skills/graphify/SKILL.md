...
> **MVCore — OBRIGATÓRIO: use o entry point versionado do repositório.**
>
> Neste repositório a detecção de arquivos alterados do graphify está errada:
> `detect_incremental` escolhe UM campo de hash para o corpus inteiro
> (`kind="semantic"` por padrão), então todo arquivo de código — cujo
> `semantic_hash` é legitimamente vazio — vira "alterado". Aqui isso infla 4
> mudanças reais em ~1130.
>
> Antes de executar QUALQUER passo deste runbook (Steps 1-9, `--update`,
> `--cluster-only`), rode o comando versionado do projeto:
>
> ```bash
> python -m tools.graphify_cmd status .    # o que mudou de verdade
> python -m tools.graphify_cmd update .    # atualização incremental (AST, 0 token)
> ```
>
> Use o interpretador gravado em `graphify-out/.graphify_python`.
>
> - `update` sai com codigo 2 quando existem documentos/imagens que realmente
>   precisam de LLM, e lista quais. SO NESSE CASO caia no fluxo de subagentes
>   deste SKILL.md (Step 3B), e apenas para os arquivos listados.
> - Nunca chame `graphify.detect.detect_incremental` direto, e nunca rode o
>   Step 2/Step 9 deste arquivo para decidir o que reprocessar ou para gravar o
>   manifest: `save_manifest` reescreve a linha inteira e apagaria o estado
>   `legacy_baseline_hash` das analises legadas preservadas.
>
> A implementacao fica em `tools/graphify_cmd.py` e `tools/graphify_sync/`,
> versionados no repo. Este SKILL.md e reinstalado a cada upgrade do graphify;
> se este bloco sumir, a fonte da verdade continua sendo `tools/`.
